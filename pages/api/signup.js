...
export default async (req, res) => {
  ...
  const user = { _id: nanoid(), username, email, password: hashedPwd, refreshToken, total: 0, win: 0, advanced: 0, avatar: 'nruwutqaihxyl7sq6ilm' }
  await db.collection('users').insertOne(user)

  const { password: pwd, ...userCopy } = user;
